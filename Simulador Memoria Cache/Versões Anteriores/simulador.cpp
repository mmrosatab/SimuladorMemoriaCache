		...
		void botaoConfigurar(wxCommandEvent& event)
	{
		...
		else
    	{
	    	...
	    	}else if(cbox->GetValue() == wxT("Assoc. Conj"))
	    	{
	    		...
	    		int nVias = cboxVias->GetValue();

	    		if(cboxVias->GetValue() == "2")
	    		{
	    			// numero de colunas fixo eh 7, add + 2 para vias
	    			cout <<"oxe"<< endl;
	    			//grade->InsertCols(7,2,true);
				...
				grade->SetColLabelValue(7,  wxString::FromUTF8("V0"));
				grade->SetColLabelValue(8,  wxString::FromUTF8("V1"));
				...
				grade->SetColSize(7,20);
				grade->SetColSize(8,20);
	    	}
